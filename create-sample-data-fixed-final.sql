...
-- Step 3: 기존 사용자들을 테스트 데이터로 변환 (수정된 방법)
...
update_data AS (
    SELECT 
        ...
        END as new_name,
        CASE 
            WHEN rn <= 5 THEN 'individual'
            WHEN rn <= 9 THEN 'enterprise'
            ELSE 'individual'
        END as new_account_type,
        CASE 
            WHEN rn = 6 THEN '(주)테크코프'
            WHEN rn = 7 THEN '스타트업허브'
            WHEN rn = 8 THEN '이노베이트'
            WHEN rn = 9 THEN '퓨처테크'
            ELSE NULL
        END as new_company,
        CASE rn
            ...
            ELSE '테스트 직책'
        END as new_position,
        CASE WHEN rn % 3 = 0 THEN false ELSE true END as new_verified,
        '010-' || LPAD((1000 + rn * 111)::text, 4, '0') || '-' || LPAD((5000 + rn * 123)::text, 4, '0') as new_phone
    FROM numbered_users
    WHERE rn <= 9
)
UPDATE users 
SET 
    ...
    account_type = update_data.new_account_type,
    company = update_data.new_company,
    position = update_data.new_position,
    verified = update_data.new_verified,
...
expert_users AS (
    SELECT u.id, ROW_NUMBER() OVER (ORDER BY u.created_at) as expert_rn
    FROM users u
...
SELECT 
    r.id,
    CASE WHEN r.rn <= 2 THEN (
        SELECT e.id FROM expert_users e WHERE e.expert_rn = ((r.rn - 1) % 2) + 1
    ) ELSE NULL END,
    CASE r.rn
