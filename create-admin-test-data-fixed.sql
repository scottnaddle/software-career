-- 관리자 대시보드 테스트를 위한 구체적인 테스트 데이터 생성 (수정버전)
-- 기존 사용자만 사용하여 Foreign Key Constraint 에러 방지
...
BEGIN
    ...
    RAISE NOTICE '현재 비관리자 사용자 수: %', current_user_count;
    
    IF current_user_count < 6 THEN
        RAISE NOTICE '테스트 데이터 생성을 위해 최소 6명의 사용자가 필요합니다.';
        RAISE NOTICE '현재 %명의 사용자가 있습니다. 추가로 %명의 사용자가 필요합니다.', current_user_count, (6 - current_user_count);
        RAISE NOTICE '웹 애플리케이션에서 회원가입을 통해 추가 사용자를 생성해 주세요.';
    ELSE
        RAISE NOTICE '충분한 사용자가 있습니다. 테스트 데이터를 생성합니다.';
    END IF;
END $$;
